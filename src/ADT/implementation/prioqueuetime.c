...
#include <stdio.h>
#include <stdlib.h>
#include "../headers/prioqueuetime.h"
...
void Enqueue (PrioQueueTime * Q, infotype X){
	if(!IsEmpty_PRIOQUEUE(*Q)){
		...
		while (i != Head(*Q))
		{
			if (Time(Elmt(*Q,i)) < Time(Elmt(*Q, (i - 1 + MaxEl(*Q)) % MaxEl(*Q)))){
				temp = Elmt(*Q, i);
				Elmt(*Q, i) = Elmt(*Q, (i - 1 + MaxEl(*Q)) % MaxEl(*Q));
				Elmt(*Q, (i - 1 + MaxEl(*Q)) % MaxEl(*Q)) = temp;
			}
			i = (i - 1 + MaxEl(*Q)) % MaxEl(*Q);
		}
	} else {
		Head(*Q) = 0;
		Tail(*Q) = 0;
